...
- **결정은 PRD.md에 D-번호로 기록.** 무엇을·왜 + 근거. 현재 D-55까지 있음. 새 결정은 다음 번호로.
...
- **방금 끝남**: 활성도 사전계산 구조 전환(D-53) + 4번째 패널 '지금 뜨는 어종'(D-54). D-53은 요청마다 계산하던 활성도를 15분 수집 사이클 끝에 전 어종 한 번 계산해 인메모리 스토어(scoring._store)에 원자 교체로 담고, 요청은 `cached_card`로 조회만 하게 바꿨다(스토어가 비면 즉석 계산 폴백). 상세 페이지·라벨 스냅샷은 종전대로 즉석 계산. D-54는 그 스토어를 읽어 전 어종 활성도 상위 5를 대시보드 aside에 점수 숫자로 표시한다 — `top_active`는 **스토어만 읽고 폴백하지 않는다**(251종 즉석 계산 방지, 비면 '지금은 뜨는 어종이 없어요' 표시). D-55는 안내 문구 2종(어종 상세 제보 유도 — `can_label`인 사용자에게 / 대시보드 닉네임 안내 — 닉네임 미설정자에게만)과, 테스트 네 스위트에 `sys.exit(1 if fails else 0)`를 넣어 실패 시 `&&` 체인이 멈추게 한 것. 함께 정립: 패치노트 작성 형식(PATCHNOTE_STYLE.md)과 README 개발 변천사 전면 재작성. 그 전: 보안·계정 정비 1차(D-52) — 프로젝트 전체 검수(07-14) 후속. 라벨 15분 내 재제보는 새 행 대신 갱신(연타로 리더보드 부풀리기 차단, 판단 정정 허용), 세션 버저닝(users.session_version 자동 ALTER, 비번 변경 시 타 기기 세션 무효 — 구 포맷 토큰 무효라 배포 시 전원 1회 재로그인), 가입 시 닉네임 필수(기존 무닉네임 유저는 NULL 유지), RF4_SECRET 미설정 시 기동 거부, 로그아웃 POST화·/me 플래시 쿠키·세션 쿠키 secure. 그 전: 리더보드 옆 어종 집계 패널 2종(D-51), 마이페이지·닉네임·리더보드(D-50), 미끼 패밀리 정규화 피처(D-49), 서식 수역 맵 화면 반영(D-48).
