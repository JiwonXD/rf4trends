...
`[변경]` **어종 상세 '장소 분포 · 수역별 활성도'가 서식 수역 맵(species_waterbodies) 기준으로 전 수역을 표시(D-48).** D-46이 예고했던 화면 반영 후속.
...
### 07-09 · 마이페이지 · 닉네임 · 리더보드(대시보드 aside)

`[기능추가]` **마이페이지(/me)에서 닉네임·비밀번호 변경, 리더보드 노출 설정이 가능해졌다. 리더보드는 이번 주 스냅샷 제보 횟수 기준으로 대시보드 우측 여백에 표시된다(D-50).** 향후 리더보드 시스템 확장의 1단계.

- **닉네임은 백필하지 않음(개인정보 보호)**: id 규칙에 `_ - @ .`을 허용해 이메일 형태 아이디가 가능해졌는데, 이를 그대로 공개 닉네임으로 노출하면 개인정보 유출 위험 — 기존·신규 유저 모두 닉네임은 빈 값으로 시작하고, 마이페이지에서 스스로 등록해야만 리더보드에 표시된다.
- **리더보드는 별도 페이지가 아니라 대시보드 우측의 빈 여백을 활용**한다(넓은 화면에서만 300px 패널 추가, 기존 본문 폭 860px는 그대로 — 내용 축소 없음). 960px 이하 화면에서는 카드 아래로 자연스럽게 내려간다. 접속 즉시 눈에 들어와야 한다는 목적(활성도 추천과 같은 맥락)상 클릭해서 들어가는 페이지보다 이 구조가 맞다고 판단.
- 이번 주(월 04:00 KST 리셋 이후) 라벨 제보 횟수 상위 10명. admin 계정과 비공개·닉네임 미등록 유저는 제외. **동점 시 그 주 마지막 제보가 더 이른 사람이 윗순위** — 리스트 표시 순위와 "내 순위" 계산이 같은 함수(labels.weekly_ranking)를 공유해 서로 어긋나지 않는다.
- 활동량(횟수)만으로 순위를 매기는 이유: 스냅샷의 "가치"까지 평가하는 알고리즘은 라벨이 아직 충분히 쌓이지 않아 설계 근거가 부족하다 — 데이터가 쌓이면 점수제 전환을 검토한다.
...
`[변경]` **프로젝트 전체 검수에서 "리더보드가 경쟁 인센티브를 만들었는데 악용 방지는 개인 도구 수준"이 최상위 리스크로 지목되어, 라벨 연타 방지·세션 버저닝·가입 시 닉네임 필수·기동 게이트·쿠키 하드닝을 한 번에 정비(D-52).**

- **라벨 재제보는 15분 내 갱신 처리**: 같은 (유저, 어종, 수역, 시간창)으로 15분 안에 다시 제보하면 새 행 대신 기존 행을 갱신(스냅샷 재박제). 데이터가 15분 주기로만 바뀌므로 같은 주기 내 재제보는 새 정보가 아니라는 판단 — 연타로 리더보드 횟수를 부풀리는 걸 막으면서 판단 정정은 허용한다. 스팸 라벨은 순위 조작이자 곧 학습 데이터 오염이라 모델 품질 문제이기도 함.
- **세션 버저닝**: 토큰에 users.session_version을 함께 서명하고 비밀번호 변경 시 +1 — 비밀번호를 바꾸면 다른 기기의 세션이 전부 풀린다(현재 기기는 쿠키 재발급으로 유지). 종전에는 서명 토큰만 있어 유출된 세션을 회수할 방법이 없었다. 구 포맷 토큰은 자동 무효라 배포 시 전원 1회 재로그인.
